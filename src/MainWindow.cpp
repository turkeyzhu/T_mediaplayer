...
//    mPopMenu->addSeparator();       //添加分离器
    ...
    pdragWidget =new DragWidget/*(ui->listWidget)*/;
    pdragWidget->move(ui->listWidget->pos());

    pdragWidget->setToolTip(QString::fromUtf8("--媒体列表--"));
    connect(pdragWidget, SIGNAL(addFilePath(QString)), this, SLOT(add_File_Path(QString)));   //turkeyzhu
//    connect(pdragWidget, SIGNAL(itemDoubleClicked(QListWidgetItem *item)), ui->listWidget,SIGNAL(itemDoubleClicked(QListWidgetItem *item)));
    connect(pdragWidget, &QListWidget::customContextMenuRequested, ui->listWidget, &QListWidget::customContextMenuRequested);
    pdragWidget->setContextMenuPolicy(Qt::CustomContextMenu);  //turkeyzhu
...
//    ui->page_video->installEventFilter(this);
    ...
    resize(1024,556);   //turkeyzhu for nice height
//    setTitle(QStringLiteral("我的播放器-V%1").arg(AppConfig::VERSION_NAME));
    setTitle(QStringLiteral("TurkeyPlayer-V1.1"));

    mVolume = mPlayer->getVolume();


    ui->listWidget->setDragEnabled(true);   //turkeyzhu
    ...
    pdragWidget->setDragEnabled(true);
    pdragWidget->setMovement(QListView::Free);
    pdragWidget->viewport()->setAcceptDrops(true);
    pdragWidget->setDragDropMode(QAbstractItemView::InternalMove);


   //qDebug()<<"--------ui->widget_container->children--------------------"<< ui->widget_container->children();
...
void MainWindow::showOutControlWidget()
{
    ...
    int x = 0;
    int y = ui->widget_container->height() - h; //turkiyezhu rewidth
//    qDebug()<<"----------- ui->widget_container->height()---"<< ui->widget_container->height()<<"--control_height---"<<h;
...
//    qDebug()<<"------filePath----"<<filePath<<"-------AppConfig::gVideoFilePath----"<<AppConfig::gVideoFilePath;
...
void MainWindow::addVideoFile(const QString &filePath)
{
    ...
    if(!fileInfo.exists())  //turkeyzhu add for check is invalid file
    {
        return;
    }
...
//    pdragWidget->addItem(item);  //turkeyzhu
...
void MainWindow::add_File_Path(QString filePath)
{
//    QStringList l;
//    l<<filePath;
//    addVideoFiles(l);
...
//        dialog.setVideoUrl(AppConfig::gVideoFilePath);
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        qDebug()<<"[T_player]--stream-url:["<<s<<"]--"<<endl;
...
{
    if (QObject::sender() == ui->listWidget || QObject::sender() ==pdragWidget) //turkeyzhu
    {
...
void MainWindow::onOpenSdlFailed(const int &code)
{
    FunctionTransfer::runInMainThread([=]()
    {
       // QMessageBox::critical(NULL, "SDL_ERROR", QString("open Sdl failed %1").arg(code));
            qDebug()<<"[SDL]-----open Sdl failed!------------";
    });
}
...
qDebug()<<__FUNCTION__<<state<<mIsNeedPlayNext;
        ...
        {
            ui->stackedWidget->setCurrentWidget(ui->page_open); //turkeyzhu for first statck widget
//            ui->stackedWidget->setCurrentWidget(ui->page_video);
            ui->pushButton_pause->hide();
             ui->pushButton_play->show();  //turkeyzhu
...
{
   // qDebug()<<"MainWindow::eventFilter----------"<<target<<"--------"<<event<<endl;
    ...
    else if(target==ui->listWidget)
    {
        ...
        {
            //qDebug()<<"ui->listWidget-----------enter---2-";
            QDragEnterEvent *dee = dynamic_cast<QDragEnterEvent *>(event);
            dee->acceptProposedAction();
            QString itdee=dee->mimeData()->text();
            staticpath[0]=itdee;
            staticpath[0]=staticpath[0].replace("file:///","");
            qDebug()<<"ui->listWidget-----------enter---file-"<<itdee;
//            if(staticpath[1]!=staticpath[0])
//            {
//               staticpath[1]= staticpath[0];
//               add_File_Path(staticpath[1]);
//            }
        }
        else if(event->type()== QEvent::HoverEnter) //here is no the shoot event ,real event is Drop
        {
            //qDebug()<<"ui->listWidget-----------drop---enter-";
            if(staticpath[1]!=staticpath[0])
            {
               staticpath[1]= staticpath[0];
               add_File_Path(staticpath[1]);
    ...
    {
        //turkeyzhu
        //这个程序就是很烂，拖拽文件到列表本身就是不行，
        //重新用第三类dragwidget能实现拖拽，但是点击处理还差，
        //因为主界面和列表就不能获取到拖拽释放事件，差好多，不过这个工程目前播放可以,
        //现在虽然功能实现了，播放策略也做了处理，代码还是太冗余了，不过学习ffmpeg是可以的
...
void MainWindow::dragEnterEvent(QDragEnterEvent *event)   //turkey add for drag outside items
{
//      qDebug()<<"MainWindow::dragEnterEven---------------drag-----"<<endl;
